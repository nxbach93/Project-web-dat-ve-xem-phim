...
elseif ($role === 'staff' && $data['IDQuyen'] == 2) {
    ...
    echo "<script>alert('Đăng nhập nhân viên thành công'); window.location.href='../TrangChu/formTrangChu.php';</script>";
}
elseif ($data['IDQuyen'] == 3) {
    $_SESSION['username'] = $user;
    $_SESSION['role'] = 'admin';
    echo "<script>alert('Đăng nhập Admin thành công'); window.location.href='../TrangChu/formTrangChu.php';</script>";
}
else {
